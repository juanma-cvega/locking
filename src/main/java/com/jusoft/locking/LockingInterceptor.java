...
/**
 * Aspect in charge of finding parameters annotated with
 * {@see com.iggroup.wt.majormarketmovements.locking.LockOn}. The annotated
 * parameter will be used to synchronized the method on the object annotated. If
 * specified in the annotation, a field from a complex object can be used as the
 * monitor. This annotation can only be set in one parameter, it doesn't matter
...
public class LockingInterceptor {
	...
	private String findAnnotationValue(Annotation[][] annotations) {
		for (int i = 0; i < annotations.length; i++) {
			Optional<Annotation> lockOnAnnotation = asList(annotations[i]).stream()
					.filter(annotation -> annotation instanceof LockOn).findFirst();
	...
	private Object getMonitor(Object monitor, String monitorFieldName)
			throws NoSuchFieldException, IllegalAccessException {
		if (EMPTY.equals(monitorFieldName)) {
			return monitor;
		}
